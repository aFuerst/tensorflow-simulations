...
def _particle_electrostatic_force(simul_box, ion_dict):
    ...
    with tf.compat.v1.name_scope("particle_electrostatic_force"):
        ...
        h1_z = tf.math.reduce_sum(input_tensor=h1_z, axis=1, keepdims=True)
        ...
        h1 = tf.math.reduce_sum(input_tensor=a * b[:,:,tf.newaxis], axis=1, keepdims=False, name="sum_a_times_b") #TODO: remove need for newaxis here  #-------------->>>>> change axis here to 0
...
def _electrostatic_wall_force(simul_box, ion_dict, wall_dictionary):
    ...
    with tf.compat.v1.name_scope("electrostatic_wall_force"):
        ...
        h1_z = tf.math.reduce_sum(input_tensor=h1_z, axis=1, keepdims=True, name="sum_h1_z")
