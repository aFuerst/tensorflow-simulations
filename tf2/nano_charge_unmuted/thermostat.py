...
def update_xi_at(therms, j, dt, ke):
    ...
    if (j != 0):
        therms[j]["xi"] = therms[j]["xi"] * tf.math.exp(-0.5 * dt * therms[j + 1]["xi"]) + 0.5 * dt * (1.0 / _therm_constants[j]["Q"]) * (_therm_constants[j - 1]["Q"] * therms[j - 1]["xi"] * therms[j - 1]["xi"] - _therm_constants[j]["dof"] * utility.kB * _therm_constants[j]["T"]) * tf.math.exp(-0.25 * dt * therms[j + 1]["xi"])
    else:
...
def update_eta(therms, dt: float):
    with tf.compat.v1.name_scope("update_eta"):
        for j in range(0, len(therms)):
            if _therm_constants[j]["Q"] == 0:
                continue
            therms[j]["eta"] = therms[j]["eta"] + common.my_tf_round((0.5 * dt * therms[j]["xi"]), 6)
        return therms

def calc_exp_factor(therms, dt):
    return common.my_tf_round(tf.math.exp(-0.5 * dt * therms[0]["xi"]), 6)
