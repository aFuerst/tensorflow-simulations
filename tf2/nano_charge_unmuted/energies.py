...
def ion_energy(ion_dict, simul_box):
    ...
    with tf.compat.v1.name_scope("particle_electrostatic_energy"):
        ...
        return common.my_tf_round(tf.math.reduce_sum(input_tensor=(con) * utility.scalefactor), 6)
...
def _lj_energy(ion_dict, simul_box):
    ...
    with tf.compat.v1.name_scope("particle_lj_energy"):
        ...
        return common.my_tf_round(0.5*tf.math.reduce_sum(input_tensor=tf.math.reduce_sum(input_tensor=slice_energies, axis=1)), 6)
...
def _left_wall_lj_energy(ion_dict, simul_box):
    ...
    with tf.compat.v1.name_scope("left_wall_lj_energy"):
        ...
        return common.my_tf_round(tf.math.reduce_sum(input_tensor=tf.compat.v1.where_v2(mask[:, tf.newaxis], ulj_masked, _tf_zero, name="lj_wall_bulk_cutoff")),6)
...
def _right_wall_lj_energy(ion_dict, simul_box):
    ...
    with tf.compat.v1.name_scope("right_wall_lj_energy"):
        ...
        return common.my_tf_round(tf.reduce_sum(input_tensor=tf.compat.v1.where_v2(mask[:, tf.newaxis], d_cut, _tf_zero, name="lj_wall_bulk_cutoff")),6)
...
def _electrostatic_wall_energy(simul_box, ion_dict, wall_dictionary):
    with tf.compat.v1.name_scope("electrostatic_wall_energy"):
        ...
        return common.my_tf_round(tf.math.reduce_sum(input_tensor=fqq + fqq_csh) * utility.scalefactor, 6)
