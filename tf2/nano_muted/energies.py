...
def ion_energy(ion_dict, simul_box):
    ...
    """
    with tf.compat.v1.name_scope("particle_electrostatic_energy"):
        ...
        fqq_csh_sum = tf.math.reduce_sum(input_tensor=fqq_csh, axis=1, keepdims=True)
        ...
        fqq_sum = tf.math.reduce_sum(input_tensor=fqq, axis=1, keepdims=False)
        ...
        return tf.math.reduce_sum(input_tensor=(con) * utility.scalefactor)
...
def _lj_energy(ion_dict, simul_box):
    ...
    ion-ion
    """
    with tf.compat.v1.name_scope("particle_lj_energy"):
        ...
        return 0.5*tf.math.reduce_sum(input_tensor=tf.math.reduce_sum(input_tensor=slice_energies, axis=1))
...
def _left_wall_lj_energy(ion_dict, simul_box):
    ...
    """
    with tf.compat.v1.name_scope("left_wall_lj_energy"):
        ...
        return tf.math.reduce_sum(input_tensor=tf.compat.v1.where_v2(mask[:, tf.newaxis], d_cut, _tf_zero, name="lj_wall_bulk_cutoff"))
...
def _right_wall_lj_energy(ion_dict, simul_box):
    ...
    """
    with tf.compat.v1.name_scope("right_wall_lj_energy"):
        mask = ion_dict[interface.ion_pos_str][:, -1] > ((0.5 * simul_box.lz) - ion_dict[interface.ion_diameters_str])  # TODO: remove this mask if not cause of sim error
...
def _right_wall_columb_energy(ion_dict, simul_box):
    ...
    """
    with tf.compat.v1.name_scope("electrostatic_right_wall_energy"):
...
def _left_wall_columb_energy(ion_dict, simul_box):
    ...
    """
    with tf.compat.v1.name_scope("electrostatic_left_wall_energy"):
...
def _electrostatic_wall_energy(simul_box, ion_dict, wall_dictionary):
    with tf.compat.v1.name_scope("electrostatic_wall_energy"):
        ...
        fqq_csh = tf.math.reduce_sum(input_tensor=fqq_csh_ion, axis=1, keepdims=True)
        ...
        fqq = tf.math.reduce_sum(input_tensor=fqq_ion, axis=1, keepdims=True)
        # h1 = tf.math.reduce_sum(a * b[:, :, tf.newaxis], axis=1, keepdims=False,
        #                         name="sum_a_times_b")  # TODO: remove need for newaxis here  #-------------->>>>> change axis here to 0
        # fqq_xy = fqq[:, 0:2]  # TODO: replace this junk with better impl
        # c = fqq[:, 2:3] + fqq_csh
        # con = tf.concat(values=[fqq_xy, c], axis=1, name="x_y_and_c_concatenate")
        return tf.math.reduce_sum(input_tensor=(fqq + fqq_csh) * utility.scalefactor)


#TF version
def kinetic_energy(ion_dict):
    with tf.compat.v1.name_scope("kinetic_energy"):
        m = tf.norm(tensor=ion_dict[velocities.ion_vel_str],ord='euclidean', axis=1, keepdims=False)
        ...
        ke_sum = tf.reduce_sum(input_tensor=ke_particle, keepdims=False)
        # ke_sum_out = tf.Print(ke_sum, [ke_sum],"ke_sum")
        return common.my_tf_round(ke_sum, 3)
...
def energy_functional(box, charge_meshpoint, ion_dict):
    ...
    potential = _lj_energy(ion_dict, box) +_left_wall_lj_energy(ion_dict, box) + _right_wall_lj_energy(ion_dict, box) #ion_energy(ion_dict, box) + _lj_energy(ion_dict, box) + _left_wall_lj_energy(ion_dict, box) + _right_wall_lj_energy(ion_dict, box) + coulomb_rightwall + coulomb_leftwall
