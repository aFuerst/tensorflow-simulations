...
def velocity_verlet(curr_iter, vel, pos, force, edges_half, neg_edges_half, edges, delta_t, forces_zeroes_tf, ljatom_mass_tf, ljatom_diameter_tf, pe_graph, ke_graph):
    with tf.compat.v1.name_scope("run_one_iter"):
        if(curr_iter==0):
            ...
            #force =forces.lj_force(pos_graph, edges_half, neg_edges_half, edges, forces_zeroes_tf, ljatom_diameter_tf)
...
# Having curr_iter as an arg causes all the retracings which are expensive. 
@tf.function 
def simple_velocity_verlet(vel, pos, force, edges_half, neg_edges_half, edges, delta_t, forces_zeroes_tf, ljatom_mass_tf, ljatom_diameter_tf, pe_graph, ke_graph):
    with tf.compat.v1.name_scope("run_one_iter"):
        # if(curr_iter==0):
        #     force = forces.lj_force(pos, edges_half, neg_edges_half, edges, forces_zeroes_tf, ljatom_diameter_tf)
        #     #force =forces.lj_force(pos_graph, edges_half, neg_edges_half, edges, forces_zeroes_tf, ljatom_diameter_tf)
        #     pe_graph = energies.potential_energy(pos, edges_half, neg_edges_half, edges, ljatom_diameter_tf)
        #     ke_graph = energies.kinetic_energy(vel, ljatom_mass_tf)
        vel = update_vel(vel, force, delta_t, ljatom_mass_tf)
        pos = update_pos(pos, vel, edges_half, neg_edges_half, edges, delta_t)
        force = forces.lj_force(pos, edges_half, neg_edges_half, edges, forces_zeroes_tf, ljatom_diameter_tf)
        pe = energies.potential_energy(pos, edges_half, neg_edges_half, edges, ljatom_diameter_tf)
        vel = update_vel(vel, force, delta_t, ljatom_mass_tf)
        ke = energies.kinetic_energy(vel, ljatom_mass_tf)
        return vel, pos, force, pe, ke
...
# Can make this a tf.function? What will this do to debugging?
# @tf.function 
...
#    v_g, p_g, f_g, pe_g, ke_g =

    # writer = tf.compat.v1.summary.FileWriter(subfolder)
    # writer.add_graph(sess.graph)
    #comp_start = time.time()
    #concrete_vv = velocity_verlet.get_concrete_function(curr_iter, velocities, positions, forces, edges_half, neg_edges_half, edges, delta_t, forces_zeroes_tf, ljatom_mass_tf, ljatom_diameter_tf, potentials, kinetics)
    #print(velocity_verlet.pretty_printed_concrete_signatures())
    
    for curr_iter in range(steps):
        # Would help with the retracing if all these are tensors and not generic python objects?
        velocities, positions, forces, potentials, kinetics = simple_velocity_verlet(velocities, positions, forces, edges_half, neg_edges_half, edges, delta_t, forces_zeroes_tf, ljatom_mass_tf, ljatom_diameter_tf, potentials, kinetics)
        ...
        #velocities, positions, forces, potentials, kinetics = 
        #sess.run([v_g, p_g, f_g, pe_g, ke_g], feed_dict=feed_dict, options=run_options, run_metadata=run_metadata)
        # Above 3 for initialization? No! This is actually calling the velocity verlet operation!! 
        ...
        # PRATEEK: Fix this hit_eqm and logging: results and behavior should be independent of logging frequency. Otherwise it crashes for small s
        ...
        # PRATEEK: Why are these zeroed out? Do they need to be reset on every iteration? 
        potentials = np.zeros((1), dtype=df_type)
        kinetics = np.zeros((1), dtype=df_type)
    
    pe_stdev = 1 #statistics.stdev(avg_ke_arr)
    ke_stdev = 1 #statistics.stdev(avg_pe_arr)
    temp_stdev = 1 #statistics.stdev(map(lambda x:2*x/3,avg_ke_arr))
    cumm_avg_pe = tot_pe/(samples*number_ljatom)
    cumm_avg_ke = tot_ke/(samples*number_ljatom)
    avg_temp = (2*cumm_avg_ke)/3

    # PRATEEK: round not a TF operation 
    #print(ljatom_density, " ",round(cumm_avg_pe,3), " ", round(avg_temp,3)," ",round(pe_stdev,3)," ", round(temp_stdev,3))
    print(ljatom_density, " ",cumm_avg_pe,3, " ", avg_temp,3," ",pe_stdev,3," ", temp_stdev,3)
