...
def wrap_distances_on_edges(simul_box, distances):
    ...
    """
    with tf.compat.v1.name_scope("wrap_distances_on_edges"):
...
def magnitude(tensor, axis:int=2, keepdims:bool=False):
    ...
    with tf.compat.v1.name_scope("magnitude"):
        return my_tf_round(tf.math.sqrt(magnitude_squared(tensor, axis, keepdims)), 6)
...
def magnitude_squared(tensor, axis:int=2, keepdims:bool=False):
    ...
    if(tensor.shape[axis] != 3):
        raise Exception("Given axis value '{}' did not have a depth of 3, but was '{}'".format(axis, tensor.shape[axis]))
    with tf.compat.v1.name_scope("magnitude_squared"):
        return my_tf_round(tf.math.reduce_sum(input_tensor=tf.math.pow(tensor,2.0), axis=axis, keepdims=keepdims),6)
...
def wrap_vectorize(fn, elems):
    return my_tf_round(tf.function(lambda: tf.compat.v1.vectorized_map(fn=fn, elems=elems))(),6)
