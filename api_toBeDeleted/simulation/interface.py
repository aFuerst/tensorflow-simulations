...
from util.common import py_array_to_np as conv
...
class Interface:
    ...
    def __init__(self, salt_conc_in, salt_conc_out: float, salt_valency_in: int, salt_valency_out: int, bx: float,
                 by: float, bz: float, initial_ein: float = 80, initial_eout: float = 80):
        ...
        if (salt_conc_in != 0):
            self.inv_kappa_in = (0.257 / (salt_valency_in * math.sqrt(
                self.lB_in * utility.unitlength * salt_conc_in))) / utility.unitlength
            self.mean_sep_in = pow(1.2 * salt_conc_in, -1.0 / 3.0) / utility.unitlength
        else:
            self.inv_kappa_in = 0
            self.mean_sep_in = 0
        if (salt_conc_out != 0):
            self.inv_kappa_out = (0.257 / (salt_valency_out * math.sqrt(
                self.lB_out * utility.unitlength * salt_conc_out))) / utility.unitlength
    ...
    def put_saltions_inside(self, pz: int, nz: int, concentration: float, positive_diameter_in: float,
                            negative_diameter_in: float, counterions: int, valency_counterion: int,
                            counterion_diameter_in: float, bigger_ion_diameter: float, crystal_pack: bool):
        ...
        total_nions_inside = int(
            (concentration * 0.6022) * (volume_box * utility.unitlength * utility.unitlength * utility.unitlength))
                ...
                for j in range(num_ions_in_ly):
                    ...
                    for k in range(num_ions_in_lz):
                        if len(ion_pos) < total_saltions_inside:
                            ...
                            if (x > ((self.lx / 2) - (0.5 * bigger_ion_diameter)) or y > (
                                    (self.ly / 2) - (0.5 * bigger_ion_diameter)) or z > (
        ...
        return {ion_pos_str: conv(ion_pos), ion_charges_str: conv(ion_charges), \
                ion_masses_str: conv(ion_masses), ion_diameters_str: conv(ion_diameter),
                ion_epsilon_str: conv(ion_epsilon),
                ion_valency_str: conv(ion_valency)}  # , ion_valency_str:conv(ion_valency)}
    ...
    def discretize(self, smaller_ion_diameter: float, f: float, charge_meshpoint: float):
        ...
        for j in range(ny):
            for i in range(nx):
                position = conv([-0.5 * self.lx + 0.5 * smaller_ion_diameter + i * self.width,
                                 -0.5 * self.ly + 0.5 * smaller_ion_diameter + j * self.width, -0.5 * self.lz])
                normal = conv([0, 0, -1])
    ...
    def electrostatics_between_walls(self):
        ...
        fqq_walls = 0.5 * self.right_plane["q"] * self.left_plane["q"] * 0.5 * (
                    1.0 / self.right_plane["epsilon"] + 1.0 / self.left_plane["epsilon"]) / r_vec_walls_magnitude
