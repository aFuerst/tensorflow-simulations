...
class Interface:
    ...
    def discretize(self, smaller_ion_diameter: float, f: float, charge_meshpoint: float):
        ...
        f_planes = open(os.path.join("leftplane.xyz"),"a")
        # creating a discretized hard wall interface at z = - l/2
        f_planes.write("ITEM: TIMESTEP"+"\n")
        f_planes.write("0 \n"+"ITEM: NUMBER OF ATOMS \n"+str(nx*ny)+"\n"+"ITEM: BOX BOUNDS \n")
        f_planes.write(str(-0.5*self.lx)+"\t"+str(0.5*self.lx)+"\n")
        f_planes.write(str(-0.5*self.ly)+"\t"+str(0.5*self.ly)+"\n")
        f_planes.write(str(-0.5*self.lz)+"\t"+str(0.5*self.lz)+"\n")
        f_planes.write("ITEM: ATOMS index type x y z \n")
        ...
        for j in range(ny):
            for i in range(nx):
                ...
                f_planes.write(str(i*j)+"\t"+"1"+"\t"+str(left_plane["posvec"][i*j-1][0])+"\t"+str(left_plane["posvec"][i*j-1][1])+"\t"+str(left_plane["posvec"][i*j-1][2]-0.5*smaller_ion_diameter)+"\n")
        f_planes.close()

        # creating a discretized hard wall interface at z = l/2
        f_planes = open(os.path.join("rightplane.xyz"), "a")
        f_planes.write("ITEM: TIMESTEP" + "\n")
        f_planes.write("0 \n" + "ITEM: NUMBER OF ATOMS \n" + str(nx * ny) + "\n" + "ITEM: BOX BOUNDS \n")
        f_planes.write(str(-0.5 * self.lx) + "\t" + str(0.5 * self.lx) + "\n")
        f_planes.write(str(-0.5 * self.ly) + "\t" + str(0.5 * self.ly) + "\n")
        ...
        for j in range(ny):
            for i in range(nx):
                ...
                f_planes.write(str(i * j) + "\t" + "1" + "\t" + str(right_plane["posvec"][i * j - 1][0]) + "\t" + str(
                    right_plane["posvec"][i * j - 1][1]) + "\t" + str(
                    right_plane["posvec"][i * j - 1][2] + 0.5 * smaller_ion_diameter) + "\n")
        f_planes.close()
