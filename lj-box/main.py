...
def calc_force(pos):
    ...
    def single_force(atom_pos):
        ...
        # can't see a way to remove that case in every above computation, easier to do it all at once at the end
        filter = tf.math.logical_or(tf.math.is_inf(slice_forces), tf.math.is_nan(slice_forces))
        slice_forces = tf.where(filter, forces_zeroes_tf, slice_forces)
        # slice_forces = tf.debugging.check_numerics(slice_forces, "slice_forces")
  
        return tf.math.reduce_sum(slice_forces, axis=0)
